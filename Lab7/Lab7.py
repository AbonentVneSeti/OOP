...
def demo(di_container: Injector) -> None:
    ...
    with di_container.scope():
        ...
        print(f"Same Logger instance in and outside of scope: {logger1 is logger2}")
    ...
    print(f"Same DB instance outside of scope: {db1 is db3}")
    if di_container._registrations[EmailServiceProtocol]['life_style'] == LifeStyle.PER_REQUEST:
        with di_container.scope():
            ...
            print(f"Same EmailService instances with PerRequest: {email2 is email3}")
